...
class BrassDeck
{
	...
	{
		Permutation p = PermutationFactory.getPermutation("resources/brass_deck_shuffle.txt", 66, 66);
		ArrayList<BrassCard> shuffle = new ArrayList<BrassCard> ();
		while (p.hasNext())
		{
			int q= p.next();
			BrassCard bc = brass_deck.get(q);
			shuffle.add(bc);
		}
	...
	{
		BrassCard bc = shuffled_deck.remove(0);
		return bc;
	}
